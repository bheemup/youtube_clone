import React from 'react';
import "../App.css"

function Filters() {
  return (
    <div className="filters">
          
              <button className="filter_option">All</button>
              <button className="filter_option">Mixed</button>
              <button className="filter_option">Music</button>
              <button className="filter_option">Javascript</button>
              <button className="filter_option">Dubbing</button>
              <button className="filter_option">Computer</button>
              <button className="filter_option">CSS</button>
              <button className="filter_option">HTML</button>
              <button className="filter_option">HTML</button>
              <button className="filter_option">HTML</button>
              <button className="filter_option">HTML</button>
              <button className="filter_option">HTML</button>
              <button className="filter_option">More</button>
              <button className="slider">{">"}</button>
           </div>
  )
}

export default Filters